...
public class TopViewOfTreeByUtube {
    class Node {
        Node left, right;
        int data;
    ...
    void topViewDict(Node root, int traverseIndex) {
        if (root == null)
            return;
        nodes.putIfAbsent(traverseIndex, root);
        if (root.left != null)
        ...
        if (!levelTraverse.isEmpty()) {
            ...
            topViewDict(val.root, val.pos);
        } else
            print();
    }

    public static void main(String[] args) {
        TopViewOfTreeByUtube obj = new TopViewOfTreeByUtube();
        Node root = obj.createNode();
        obj.topViewDict(root, 0);
    }
